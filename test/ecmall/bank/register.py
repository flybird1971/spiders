...
def readIDcard():
    ...
    idCard = None
    while True:
        id = raw_input(u"请输入你身份证号 或者 输入q退出注册 : ")
        if id == 'q':
            raise MyException,"退出注册"
        regex = re.compile(r'\d{16,18}')
        if not regex.match(id):
            print u"warn : 你输入的身份证号格式不正确，请重新输入 或者 输入q退出注册 ！"
            continue
        idCard = id
        break
    return idCard
...
def readPasswd():
    ...
    """
     passwd = None
     while True:
        passwdTmp = raw_input(u"请输入密码 或者 输入q退出注册 : ")
        if passwdTmp == 'q':
            raise MyException,"退出注册"
        regex = re.compile(r'\w{6,15}')
        if not regex.match(passwdTmp):
            print u"warn : 密码6-15位，只能是字母数字下划线，请重新输入 或者 输入q退出注册 ！"
            continue
        passwd = passwdTmp
        break
     return passwd
...
def apply():
    ...
    try:
        ...
        sql.writeData({'cardNo':cardNo,'idCard':idCard,'password':passwd}) #写库
    except MyException,e:
        print e
        return False
    except IOError:
        print u"文件不存在 : %s " % (path)
        return False
